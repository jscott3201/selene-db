...
| `core_change_diff/property_diff_set_removed_128_each_sorted` | 3.1609 µs (quick) | Build a disjoint `PropertyDiff` with 128 canonical set entries and 128 canonical removed keys; keeps the positive overlap-check path visible without expecting an error. |
...
| `core_change_diff/label_diff_added_removed_50_each_sorted` | 791.95 ns (quick) | Build a disjoint `LabelDiff` with 50 canonical added labels and 50 canonical removed labels; isolates sorted disjoint-check overhead. |
