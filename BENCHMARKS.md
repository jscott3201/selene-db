...
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_64_dim128_k10_1000` | 4.07 µs (quick) | Cached `CALL selene.vector_score_candidate_state` over one maintained 64-node candidate set; avoids caller-side node-list construction and validates the generation-checked provider path. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_repeated_8x64_dim128_k10_1000` | 33.10 µs (quick) | Eight separate cached maintained candidate-state score calls, one short-lived session per query; faster than explicit-node repeated scoring, with a batched maintained-state scorer still unimplemented. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_nodes_intersection_64_dim128_k10_1000` | 4.93 µs (quick) | Cached `CALL selene.vector_score_candidate_state_nodes` intersecting a maintained 64-node state with a 64-node explicit candidate list before exact rerank. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_nodes_intersection_repeated_8x64_dim128_k10_1000` | 40.28 µs (quick) | Eight separate cached maintained-state + explicit-node intersection calls, one short-lived session per query. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_nodes_union_128_dim128_k10_1000` | 8.30 µs (quick) | Cached maintained-state + explicit-node union producing 128 canonical candidates before exact rerank. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_nodes_state_difference_32_dim128_k10_1000` | 3.08 µs (quick) | Cached maintained-state minus explicit-node candidates, leaving 32 candidates before exact rerank. |
