...
| Broad high-dimensional cosine search where exact-like quality matters and corpus size is comfortably beyond c512 | IVF when the live/clustered quality suffix holds; `TurboQuantCosine` when compressed exact-rerank state is the reason to build the index | The same-run 2k live Codestral source-chunk row has default IVF p2 at 392.83 µs with `precbp9218`, versus TurboQuant c512 at 6.3541 ms and exact at 9.3879 ms with the same suffix. Synthetic 10k TurboQuant production rows keep full recall at c512 for d768/d1536/d3072, so TurboQuant remains the default compressed cosine path candidate, but the 2k live row does not make it the query-latency default over IVF. |
...
| Row | Median | 95% CI | Notes |
|---|---:|---:|---|
| `graph_vector_omlx_embedding_pressure/exact_graph_search/mistralai_codestral-embed-2505_2k_q16_k4_dim1536_precbp9218` | 9.3879 ms | 9.3780-9.3912 ms | Exact scan remains the precision oracle for the 2,048-document repeated source-chunk corpus. |
| `graph_vector_omlx_embedding_pressure/hnsw_graph_search/mistralai_codestral-embed-2505_2k_q16_k4_ef64_dim1536_precbp8593` | 918.78 µs | 918.19-919.58 µs | HNSW is much faster than exact/TurboQuant, but loses topic precision. |
| `graph_vector_omlx_embedding_pressure/ivf_graph_search/mistralai_codestral-embed-2505_2k_q16_k4_p2_dim1536_precbp9218` | 392.83 µs | 392.58-393.11 µs | Default IVF search width p2 is the fastest same-run broad index row and preserves the exact precision suffix on this repeated source-shaped corpus. |
| `graph_vector_omlx_embedding_pressure/turbo_quant_graph_search/mistralai_codestral-embed-2505_2k_q16_k4_c512_dim1536_precbp9218` | 6.3541 ms | 6.3431-6.3592 ms | TurboQuant c512 beats exact scan while preserving the exact precision suffix through compressed preselection plus exact rerank, but is not the latency winner once IVF is included. Criterion reported no baseline p-value for these new rows; outliers were 15% / 10% for TurboQuant / IVF. |
