...
| Bench | 100k | Notes |
|---|---:|---|
| `graph_turbo_quant_candidate_recall/cluster_cos/tq2_c256_d128_k10_recallbp1875_m3515-full50000` | 237.74 ms (quick) | Benchmark-only portable TurboQuant-style row over 100k 128-dim vectors and 16 cosine queries. It normalizes vectors, applies deterministic safe orthogonal mixing, scans packed 2-bit coordinate codes with per-vector scale correction, then exact-reranks full vectors. Memory is ~3.43 MiB vs ~48.8 MiB full vectors, but 256 candidates recover too little of the exact cosine top-k. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq2_c1024_d128_k10_recallbp8125_m3515-full50000` | 152.88 ms (quick) | Wider exact rerank improves 2-bit recall to 8125 bp without changing compressed storage, but the scalar packed-code scan is still far slower than existing packed binary and PQ rows. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq3_c256_d128_k10_recallbp2125_m5078-full50000` | 143.59 ms (quick) | The 3-bit row uses ~4.96 MiB and remains weak at 256 candidates, so bit width alone is not a quality win without a stronger codebook/scoring path. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq3_c1024_d128_k10_recallbp7500_m5078-full50000` | 147.92 ms (quick) | High-candidate 3-bit row trails the 2-bit high-candidate recall while using more memory, which argues for implementing TQ+/Lloyd-Max calibration before considering production promotion. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq4_c64_d128_k10_recallbp1875_m6640-full50000` | 144.15 ms (quick) | Narrow 4-bit row is still too narrow for this scorer even after scale correction. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq4_c256_d128_k10_recallbp6250_m6640-full50000` | 146.63 ms (quick) | The per-vector scale correction makes 4-bit quality respond to wider rerank, but latency remains dominated by scalar packed-code scoring. |
| `graph_turbo_quant_candidate_recall/cluster_cos/tq4_c1024_d128_k10_recallbp10000_m6640-full50000` | 147.91 ms (quick) | High-candidate 4-bit row reaches full recall with ~6.48 MiB compressed storage, giving the first quality-positive TurboQuant-style row. It is still about an order of magnitude slower than standalone PQ full-recall rows and far slower than packed binary, so the next research step is a faster LUT/block scorer plus TQ+/Lloyd-Max fidelity before production storage work. |
