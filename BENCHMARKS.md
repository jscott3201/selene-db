...
| Bench | 10k | Notes |
|---|---:|---|
| `graph_turbo_quant_production_dimension_projection/cluster_cos/tqcos_c1024_d128_n10k_k10_recallbp10000_m1042-full5000` | 4.1261 ms (quick) | Production `VectorIndexKind::TurboQuantCosine` row over a `SharedGraph` fixture after exact rerank moved to primary graph `VECTOR` values. The compressed index keeps no duplicate full-vector handles, and the 128-dim row preserves full recall with ~1.02 MiB TurboQuant heap versus ~4.88 MiB primary vector components. |
