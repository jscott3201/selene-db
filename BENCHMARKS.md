...
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_64_dim128_k10_1000` | 4.04 µs (quick) | Cached `CALL selene.vector_score_candidate_state` over one maintained 64-node candidate set; avoids caller-side node-list construction and validates the generation-checked provider path. |
| `procedure_vector_candidate_state/shared_cache_score_candidate_state_repeated_8x64_dim128_k10_1000` | 33.09 µs (quick) | Eight separate cached maintained candidate-state score calls, one short-lived session per query; faster than explicit-node repeated scoring, with a batched maintained-state scorer still unimplemented. |
