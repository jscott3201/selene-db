...
fn validate_store_for_config(
    ...
    if let QuantizedStore::Pq(store) = store {
        ...
        // Polysemous parity must match between the decoded codebook and the
        // embedder's config. Without this recovery-time rejection, the scorer's
        // `config && codebook_flag` gate silently disables the filter
        // (false-positive polysemous_trained: filter never active;
        // false-negative: filter never applied to permuted bytes). Both modes
        // diverge post-restore from configured behavior; surface the mismatch
        // as a SectionDecodeFailed instead.
