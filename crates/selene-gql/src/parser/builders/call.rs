...
pub(super) fn build_top_level_call(pair: Pair<'_, Rule>) -> Result<ProcedureCall, ParserError> {
    build_call_stmt(pair)
}

pub(super) fn build_pipeline_call(pair: Pair<'_, Rule>) -> Result<ProcedureCall, ParserError> {
    build_call_stmt(pair)
}

fn build_call_stmt(pair: Pair<'_, Rule>) -> Result<ProcedureCall, ParserError> {
    debug_assert_eq!(pair.as_rule(), Rule::call_stmt);
    let inner = first_child(pair)?;
    match inner.as_rule() {
        Rule::call_procedure => build_procedure_call(inner),
...
fn build_yield_items(pair: Pair<'_, Rule>) -> Result<Vec<YieldItem>, ParserError> {
    pair.into_inner()
