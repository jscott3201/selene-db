...
//! across statements.
//!
//! STEP 2 registers the 19 `algo.*` procedures. The 33 platform
...
//! `selene.text_score_nodes`) are registered here,
...
impl BuiltinProcedureRegistry {
    ...
    pub fn new() -> Self {
        ...
        // `algo.*` procedures first (handles 1..=19), then the 33 `selene.*`
        // platform built-ins (handles 20..=52), continuing the same monotonic
...
mod tests {
    ...
    fn handles_are_unique_and_one_based() {
        ...
        assert_eq!(handles, (1..=52).collect::<Vec<_>>());
    }
