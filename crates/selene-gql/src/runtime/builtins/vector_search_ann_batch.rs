...
fn graph_error(error: GraphError) -> ProcedureError {
    match error {
        ...
        GraphError::Inconsistent { reason } => ProcedureError::Internal {
            detail: format!(
                "graph inconsistency during batched approximate vector search: {reason}"
            ),
        },
        other => ProcedureError::Internal {
            detail: format!(
                "unexpected graph error during batched approximate vector search: {other}"
            ),
        },
    }
}
...
fn vector_search_error(error: VectorSearchError) -> ProcedureError {
    match error {
        ...
        VectorSearchError::BatchLengthMismatch { .. } => ProcedureError::Internal {
            detail: format!("batched ANN vector search received candidate-scoring error: {error}"),
        },
