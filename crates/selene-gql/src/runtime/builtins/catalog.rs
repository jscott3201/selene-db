...
use super::{
    ...
    json_path_contains_nodes, json_path_exists_nodes, json_path_value_nodes, property_index_stats,
    reachable_nodes, rebuild_vector_indexes, reciprocal_rank_fusion, text_index_stats, text_search,
...
pub(in crate::runtime) enum BuiltinKind {
    ...
    TextIndexStats,
    /// `selene.property_index_stats` — property index drift/cardinality stats.
    PropertyIndexStats,
    ...
    JsonPathExistsNodes,
    /// `selene.json_path_contains_nodes` — exact JSON path-containment over node properties.
    JsonPathContainsNodes,
    /// `selene.json_path_value_nodes` — exact JSON path-value search over node properties.
    JsonPathValueNodes,
    /// `selene.json_contains_candidate_nodes` — exact JSON containment over explicit node candidates.
    JsonContainsCandidateNodes,
    /// `selene.json_path_exists_candidate_nodes` — exact JSON path-existence over explicit node candidates.
    JsonPathExistsCandidateNodes,
    /// `selene.json_path_contains_candidate_nodes` — exact JSON path-containment over explicit node candidates.
    JsonPathContainsCandidateNodes,
    /// `selene.json_path_value_candidate_nodes` — exact JSON path-value search over explicit node candidates.
    ...
    TextScoreNodesBatch,
    /// `selene.text_score_candidate_state` — BM25 scoring for maintained candidate state.
    TextScoreCandidateState,
    /// `selene.text_score_candidate_state_nodes` — BM25 scoring for composed maintained state and node candidates.
...
impl BuiltinKind {
    ...
    fn signature(self) -> Vec<ProcedureParameter> {
        match self {
            ...
            Self::PropertyIndexStats => property_index_stats::signature(),
    ...
    fn output_columns(self) -> Vec<ProcedureOutputColumn> {
        match self {
            ...
            Self::PropertyIndexStats => property_index_stats::output_columns(),
    ...
    pub(in crate::runtime) fn execute_graph(
        ...
        match self {
            ...
            Self::PropertyIndexStats => property_index_stats::execute(ctx, args),
