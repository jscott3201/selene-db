...
fn return_star_rejects_group_by() {
    assert_star_group_by_rejects("RETURN * GROUP BY ()", "RETURN * cannot specify GROUP BY");
}

#[test]
fn select_star_rejects_group_by() {
    assert_star_group_by_rejects(
        "SELECT * FROM MATCH (n) GROUP BY n",
        "SELECT * cannot specify GROUP BY",
    );
}

fn assert_star_group_by_rejects(source: &str, expected: &str) {
