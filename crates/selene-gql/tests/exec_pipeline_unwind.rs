...
fn unwind_emits_one_row_per_list_element() {
    ...
    let table = execute_pipeline(&plan.pipeline, input(), &ctx).expect("unwind executes");
...
fn unwind_of_non_list_returns_data_exception() {
    ...
    let err = execute_pipeline(&[op], input(), &ctx).expect_err("unwind errors");
...
fn unwind_extends_schema_with_alias_column() {
    ...
    let table = execute_pipeline(&plan.pipeline[..1], input(), &ctx).expect("unwind executes");
