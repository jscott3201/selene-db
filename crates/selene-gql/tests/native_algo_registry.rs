...
fn show_procedures_lists_all_nineteen_algo_procedures() {
    ...
    // The registry also carries the 25 `selene.*` platform built-ins, so SHOW
    // PROCEDURES lists 44; all 19 algo names must still be present.
    assert_eq!(
        table.row_count(),
        44,
        "expected 19 algo procedures + 25 platform built-ins"
    );
    for expected in [
