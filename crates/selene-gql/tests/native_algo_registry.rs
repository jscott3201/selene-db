...
fn show_procedures_lists_all_nineteen_algo_procedures() {
    ...
    // The registry also carries the 26 `selene.*` platform built-ins, so SHOW
    // PROCEDURES lists 45; all 19 algo names must still be present.
    assert_eq!(
        table.row_count(),
        45,
        "expected 19 algo procedures + 26 platform built-ins"
    );
    for expected in [
