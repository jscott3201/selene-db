...
//!
//! Lives in `tests/` (test-harness gated) because these cases need synthetic
//! catalogs and `DbString` labels/properties. The estimators are re-exported
//! under the `test-harness` feature for exactly this purpose (mirroring
//! `optimize_summary`).
...
struct StatCatalog {
    total: Option<u64>,
    label_card: Vec<(DbString, u64)>,
    eq_card: Vec<((DbString, DbString, Value), u64)>,
    avg_bucket: Vec<((DbString, DbString), u64)>,
    composite_card: Vec<((DbString, Vec<Value>), u64)>,
    composite_avg: Vec<(DbString, u64)>,
}

impl StatCatalog {
    fn label(mut self, label: DbString, count: u64) -> Self {
        self.label_card.push((label, count));
        self
    }
    fn eq(mut self, label: DbString, property: DbString, value: Value, count: u64) -> Self {
        self.eq_card.push(((label, property, value), count));
        self
    }
    fn avg(mut self, label: DbString, property: DbString, count: u64) -> Self {
        self.avg_bucket.push(((label, property), count));
        self
    }
    fn composite(mut self, label: DbString, keys: Vec<Value>, count: u64) -> Self {
        self.composite_card.push(((label, keys), count));
        self
    }
    fn composite_avg(mut self, label: DbString, count: u64) -> Self {
...
impl IndexCatalog for StatCatalog {
    fn typed_index(&self, _t: IndexTarget, _l: DbString, _p: DbString) -> Option<TypedIndexLookup> {
        None
    }
    fn label_index(&self, _t: IndexTarget, _l: DbString) -> Option<IndexHandle> {
        None
    }
    fn composite_index(
        &self,
    ...
    fn label_cardinality(&self, _t: IndexTarget, label: DbString) -> Option<u64> {
        self.label_card
            .iter()
            .find(|(l, _)| *l == label)
            .map(|(_, c)| *c)
...
fn linear_baseline_none_without_any_stat() {
    ...
    assert_eq!(
        linear_baseline(&cat, IndexTarget::Node, db_string("X")),
        None,
...
fn equality_literal_is_exact() {
    let (person, age) = (db_string("PersonEq"), db_string("age"));
...
fn equality_parameter_uses_avg_bucket() {
    let (person, age) = (db_string("PersonParam"), db_string("ageP"));
...
fn none_propagates_when_stat_absent() {
    let cat = StatCatalog::default();
    let (person, age) = (db_string("PersonNone"), db_string("ageNone"));
    let bounds = TypedIndexBounds::Equality(int_lit(30));
    assert_eq!(
        typed_index_cost(&cat, IndexTarget::Node, person, age, &bounds),
        None
...
fn in_list_sums_literal_equalities() {
    let (person, age) = (db_string("PersonIn"), db_string("ageIn"));
    let cat = StatCatalog::default()
...
fn in_list_declines_when_one_element_missing() {
    let (person, age) = (db_string("PersonIn2"), db_string("ageIn2"));
...
fn composite_literal_is_exact() {
    let (person, a, b) = (db_string("PersonComp"), db_string("a"), db_string("b"));
    let cat =
...
fn composite_parameter_uses_avg_bucket() {
    let (person, a, b) = (db_string("PersonComp2"), db_string("a2"), db_string("b2"));
...
fn disjunctive_sums_branch_cardinalities() {
    let (a, b) = (db_string("LabelA"), db_string("LabelB"));
    let cat = StatCatalog::default()
        .label(a.clone(), 10)
        .label(b.clone(), 20);
...
fn disjunctive_declines_when_a_branch_lacks_a_stat() {
    let (a, b) = (db_string("LabelDA"), db_string("LabelDB"));
