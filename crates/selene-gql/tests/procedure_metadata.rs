...
fn show_procedures_exposes_signature_outputs_and_descriptions() {
    ...
    assert!(
        names
            .iter()
            .any(|name| name == "selene.json_path_exists_nodes")
    );
    assert!(
        names
            .iter()
    ...
    assert!(
        names
            .iter()
            .any(|name| name == "selene.text_score_candidate_state")
    );
    assert!(
        names
            .iter()
            .any(|name| name == "selene.text_score_candidate_state_nodes")
    );
    assert!(
        names
            .iter()
            .any(|name| { name == "selene.text_score_candidate_state_expanded_batch" })
    );
    assert!(
        names
            .iter()
