...
impl OmlxGqlQueryRootFixture {
    ...
    pub(crate) fn execute_root_materialize_batch_query(
        &self,
        registry: &BuiltinProcedureRegistry,
        cache: Option<Arc<CallPlanCache>>,
    ) -> BindingTable {
    ...
    fn execute_anchor_lookup_query(
        ...
        match session
            .execute_source(QUERY_ANCHOR_LOOKUP_SOURCE, registry)
            .expect("oMLX GQL query anchor lookup executes")
        {
            StatementOutput::Rows(table) => table,
            other => panic!("unexpected output: {other:?}"),
        }
    ...
    fn execute_root_rows_query(
        ...
        session.bind_parameter(istr("query_index"), Value::Int(query_index as i64));
        match session
            .execute_source(QUERY_ROOT_ROWS_SOURCE, registry)
            .expect("oMLX GQL root-row traversal executes")
        {
            StatementOutput::Rows(table) => table,
            other => panic!("unexpected output: {other:?}"),
        }
    ...
    fn execute_root_materialize_query(
        ...
        session.bind_parameter(istr("query_index"), Value::Int(query_index as i64));
        match session
            .execute_source(QUERY_ROOT_MATERIALIZE_SOURCE, registry)
            .expect("oMLX GQL root materialization executes")
        {
            StatementOutput::Rows(table) => table,
            other => panic!("unexpected output: {other:?}"),
        }
