...
impl OmlxGqlQueryRootFixture {
    ...
    fn first_query_state_intersection_count(&self) -> usize {
        self.queries.first().map_or(0, |query| {
            if self.first_query_root_count() == 0 {
                0
            } else {
    ...
    fn warm_query_root_state_cache(
        &self,
        registry: &BuiltinProcedureRegistry,
        cache: Arc<CallPlanCache>,
    ) {
    ...
    fn warm_query_root_batch_cache(
        &self,
        registry: &BuiltinProcedureRegistry,
        cache: Arc<CallPlanCache>,
    ) {
    ...
    fn gql_state_precision_basis_points(
        &self,
        registry: &BuiltinProcedureRegistry,
        cache: Option<Arc<CallPlanCache>>,
    ) -> usize {
    ...
    fn gql_batch_precision_basis_points(
        &self,
        registry: &BuiltinProcedureRegistry,
        cache: Option<Arc<CallPlanCache>>,
    ) -> usize {
