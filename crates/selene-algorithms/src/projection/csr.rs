...
pub struct ProjNeighbor {
    ...
    /// permissive behavior).
    pub weight: f64,
    /// Dense index of [`Self::node_id`] within the projection's
    /// [`RowIndex`] (ALGO-01). The CSR only ever stores projected neighbors
    /// (see `build_csr` pass-1/pass-2, which skip any neighbor outside the
    /// projection), and the `RowIndex` is immutable after projection build, so
    /// this equals `row_index.dense_of_node(node_id).unwrap()` for the lifetime
    /// of the projection. Caching it lets the centrality / community / structural
...
fn build_csr(
    ...
    for row_u32 in nodes {
        ...
        for adj in entry.iter() {
            // Capture the neighbor's dense index here (ALGO-01): pass-2 already
            // had to resolve it for the projection-membership skip, so storing it
            // costs one extra u32 write and zero extra lookups.
