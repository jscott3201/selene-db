//! Parallelism-specific betweenness adapter tests.
...
#[test]
fn betweenness_args_parses_parallelism_null_zero_positive() {
    let pack = AlgorithmsPack::new();
    let registry = registry(&pack);
    let (graph, _) = graph_with_edges(8_401, &[(0, 1), (1, 2), (2, 0)]);
    ...
    let sequential = rows(execute_ok(
        "CALL algo.betweenness('p', NULL, 0) YIELD node_id, score",
        &graph,
        &registry,
    ));
    let threaded = rows(execute_ok(
        "CALL algo.betweenness('p', NULL, 4) YIELD node_id, score",
        &graph,
        &registry,
    ));
...
#[test]
fn betweenness_args_rejects_negative_parallelism() {
    let pack = AlgorithmsPack::new();
    let registry = registry(&pack);
    let (graph, _) = graph_with_edges(8_402, &[(0, 1)]);
    build_projection(&graph, &registry, "p");

    let err = execute_result(
        "CALL algo.betweenness('p', NULL, -1) YIELD node_id",
        &graph,
        &registry,
    )
...
#[cfg(target_pointer_width = "32")]
#[test]
fn betweenness_args_rejects_overflow_parallelism() {
    let pack = AlgorithmsPack::new();
    let registry = registry(&pack);
    let graph = graph_with_config_parallelism(8_403, Value::Uint(u64::MAX));
    build_unweighted_projection(&graph, &registry, "p", &["Person"]);

    let err = execute_result(
        "MATCH (cfg:Config) CALL algo.betweenness('p', NULL, cfg.parallelism) YIELD node_id RETURN node_id",
        &graph,
        &registry,
    )
    ...
    assert_eq!(
        ...
        "algo.betweenness: parallelism is too large"
    );
}

#[test]
fn betweenness_args_rejects_above_1024_parallelism() {
    let pack = AlgorithmsPack::new();
    let registry = registry(&pack);
    let (graph, _) = graph_with_edges(8_404, &[(0, 1)]);
    build_projection(&graph, &registry, "p");

    let err = execute_result(
        "CALL algo.betweenness('p', NULL, 1025) YIELD node_id",
        &graph,
        &registry,
    )
