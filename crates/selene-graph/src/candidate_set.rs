...
#[derive(Clone)]
pub(crate) struct RuntimeLineage(Arc<RuntimeIdentity>);
...
impl RuntimeLineage {
    ...
    pub(crate) fn downgrade(&self) -> RuntimeLineageLease {
        RuntimeLineageLease(Arc::downgrade(&self.0))
    }
}

impl fmt::Debug for RuntimeLineage {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("RuntimeLineage")
    }
}

#[derive(Clone)]
pub(crate) struct RuntimeLineageLease(Weak<RuntimeIdentity>);

impl RuntimeLineageLease {
    pub(crate) fn upgrade(&self) -> Option<RuntimeLineage> {
        self.0.upgrade().map(RuntimeLineage)
    }
}
...
impl CandidateSet<Node> {
    ...
    pub(crate) fn bind_provider_ids(
        graph: &SeleneGraph,
        ids: impl IntoIterator<Item = NodeId>,
    ) -> Result<Self, NodeId> {
        let mut rows = RoaringBitmap::new();
        for id in ids {
            let Some(row) = graph.node_row_for_id(id) else {
                return Err(id);
            };
        ...
        Ok(Self::from_node_rows(graph, rows))
    }
