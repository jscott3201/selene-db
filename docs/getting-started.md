...
- `(p:Person & Engineer)` requires both labels. `(p:Person | Engineer)` requires at least one. The GQL Flagger uses the generated parser-admission disposition, which is separate from complete runtime support and formal claim state; see [the GQL reference](gql-reference.md).
...
- [GQL reference](gql-reference.md) — generated parser admission, runtime inventory, and the GQL surface.
