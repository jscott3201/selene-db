...
- Existing row-indexed APIs and private row aliases may remain unchanged in M04-PR01; after the already-completed parser CI prerequisite, M04-PR02's delivery of exactly four sequential implementation parts owns private `NodeRow`/`EdgeRow` wrappers and repository-wide removal of existing public row surfaces, with numeric Parts 3 and 4 identified as Part 3A and Part 3B.
...
After the already-completed parser CI prerequisite, deliver M04-PR02 as exactly four sequential, independently D-021-bounded implementation parts under this one work-item ID: Parts 1 and 2 retain their completed typed-identity and graph-producer scopes; numeric Part 3, titled Part 3A, deletes the graph-internal bridge while retaining only the current downstream raw-row APIs required by Part 3B; numeric Part 4, titled Part 3B, migrates downstream consumers, deletes every final public raw-row surface, supplies final evidence, and only then closes #1093 and unblocks dependents.

### Scope

- Treat the already-completed parser CI prerequisite as prerequisite evidence outside delivery; deliver this one M04-PR02 work-item ID in exactly four sequential reviewable implementation parts, each independently capped by D-021 at 25 production files and 1,500 net non-generated lines; numeric Parts 3 and 4 are titled Part 3A and Part 3B.
...
- Part 1 — lifecycle correctness: make graph mutation/publication, delete/remove/reset, recovery/open, compaction/remap, rebuild, consistency, and shared ownership preserve or remint physical layout identity exactly; retain only the named `Part 3A graph-internal lower-row bridge`, keep M04-PR02 `Unmerged`, leave #1093 open, and keep dependents blocked.
...
- Part 2 — binding and recovery: the pinned `SeleneGraph` snapshot solely binds duplicate-canonicalized unbound stable IDs and existing `VectorCandidateSet` values using liveness only, while maintained-state recovery reserves runtime attachment before recovery callbacks; retain only the named Part 3A graph-internal bridge, keep M04-PR02 `Unmerged`, leave #1093 open, and keep dependents blocked.
- `crates/selene-graph/src/recover.rs` is the required Part 2 owner for reserving provider runtime attachment before `recover_guarded` callbacks, cleanup after callback failure or cancellation, and final attachment sequencing.
...
- Existing public stable-ID slice and `VectorCandidateSet` compatibility wrappers may remain during Part 2 only if they delegate into pinned-snapshot typed binding; they are not a physical-row bridge and do not weaken the sole-binder or vector-property-skip contracts.
- The Part 2 JSON/text/vector helper paths remain Part 2 implementation owners, not an implicit extra delivery part, D-021 exception, dependency inversion, public row bridge, or compatibility promise.
...
- **Bridge/deletion state:** Only the named Part 3A graph-internal lower-row bridge—the minimum crate-private trusted row iterator/resolver needed through the exact Part 3A inventory—may remain; it cannot cross the stable facade or be advertised as compatibility, and Part 3A owns deletion.
...
- **Exact production paths:**
  ...
  - `crates/selene-graph/src/text_index/candidate.rs`
...
- **Bridge/deletion state:** Only the same named Part 3A graph-internal lower-row bridge may remain for the exact Part 3A inventory; no additional row adapter, facade crossing, or compatibility promise may survive Part 2, and Part 3A owns deletion.
- **Completion effect:** M04-PR02 remains `Unmerged`, #1093 remains open, and dependents remain blocked after Part 2.

#### Part 3 — Part 3A: Graph-internal bridge deletion

- **Outcome:** Migrate every graph-internal `trusted_rows` consumer to typed-candidate or stable-ID-safe internal flows and delete both named `CandidateSet::{Node,Edge}::trusted_rows` bridge methods without a renamed or equivalent raw-row bridge or facade crossing, while retaining only the current public downstream raw-row APIs strictly required by the exact Part 3B consumers.
- **Budgets:** at most 25 production files and 1,500 net non-generated lines.
- **Structured state after:** work item `Unmerged`; issue `Open`; dependents unblocked `false`; bridge `Deleted`.
...
- **Exact production paths:**
  ...
  - `crates/selene-graph/src/vector_search/score_candidate_batch.rs`
- **Acceptance:**
  - Every graph-internal `trusted_rows` call site uses a typed-candidate or stable-ID-safe internal flow, and both named bridge methods are deleted without a renamed/equivalent raw-row bridge or facade crossing.
  - Liveness, generation, and snapshot-layout checks, vector missing/non-vector skip behavior, deterministic result semantics, and the existing unbound stable-ID `VectorCandidateSet` remain intact.
  - Only the current public downstream raw-row APIs strictly required by exact Part 3B consumers remain; M04-PR02 stays `Unmerged`, #1093 stays open, and dependents stay blocked.
  - Focused graph read/search/index tests, measured bridge-removal/internal-candidate cost, API/lint evidence, and exact production-file/net-line accounting pass outside this exact inventory.
- **Bridge/deletion state:** The graph-internal `CandidateSet::{Node,Edge}::trusted_rows` bridge is `Deleted` and cannot regress or return under another name; the existing unbound stable-ID `VectorCandidateSet` remains outside bridge debt, while Part 3B owns the remaining public downstream row surfaces.
- **Completion effect:** After Part 3A, the structured bridge state is `Deleted`, but M04-PR02 remains `Unmerged`, #1093 remains open, and dependents remain blocked pending Part 3B.
...
- **Completion effect:** Only merged Part 3B with all final evidence closes #1093, marks M04-PR02 `Merged`, and unblocks dependents.
...
- No new work-item IDs, fifth or implicit extra implementation part, D-021 exception, or compatibility promise for the named Part 3A graph-internal bridge or remaining Part 3B public-row debt.

### Acceptance evidence

- The already-completed parser CI prerequisite remains prerequisite evidence outside the exactly four sequential implementation parts and does not become another M04-PR02 work-item ID.
...
- Part 1: graph mutation/publication, recovery/open, compaction/remap, reset, and non-remapping rebuild paths preserve or remint each `CandidateSet`'s non-reused Arc-style layout identity exactly; only the named Part 3A graph-internal lower-row bridge may remain, M04-PR02 stays `Unmerged`, #1093 stays open, and dependents stay blocked.
...
- Part 2: only the named Part 3A graph-internal lower-row bridge may remain; M04-PR02 stays `Unmerged`, #1093 stays open, and dependents stay blocked.
- Part 3A: every graph-internal `trusted_rows` consumer uses a checked typed-candidate or stable-ID-safe flow; both bridge methods are deleted without an equivalent bridge or facade crossing; liveness/generation/layout checks, vector skip behavior, deterministic semantics, and existing unbound stable-ID `VectorCandidateSet` remain correct.
- Part 3A: only the current public downstream raw-row APIs required by exact Part 3B consumers remain, the structured bridge state is `Deleted` without regression, M04-PR02 stays `Unmerged`, #1093 stays open, and dependents stay blocked.
- Part 3B: every forecast algorithm/private projection, GQL runtime/optimizer/builtin, testing adapter, and facade-boundary consumer uses typed candidates or an ID-safe resolver, with an internal-only dense projection abstraction, facade `DatabaseId` ingress validation, and no dependency inversion.
- Part 3B final: repository-public API and lint evidence contain no graph `RowIndex`, raw-row conversion/signature, row-indexed bitmap producer, repeated row→ID loop, legacy raw-row adapter, public downstream row surface, or regressed graph-internal bridge.
- Part 3B final: product-shaped algorithm/projection, optimizer/resolver, common-query, facade/testing performance is within reviewed bounds, final full/docs/API/lint evidence passes, and only then may #1093 close, M04-PR02 become `Merged`, and dependents unblock.
...
- Part 2: graph producer/index/JSON/text/vector/reachability/type-validation and lower read-path regressions prove typed candidate ownership and that only the named Part 3A graph-internal bridge remains below the stable facade.
...
- Parts 1 and 2: the named graph-internal lower-row bridge is the only remaining bridge, is limited to exact Part 3A consumers, carries explicit Part 3A deletion ownership, and creates no compatibility promise.
...
- Part 3A: graph-internal bridge deletion has no renamed/equivalent raw-row bridge, facade crossing, liveness/generation/layout regression, vector-skip regression, or nondeterministic result change; only exact Part 3B public-row debt remains.
- Part 3B: downstream algorithms/private projections/GQL/optimizer/builtins/testing/facade-boundary migration has no dependency inversion, public graph storage-row abstraction, compatibility alias, hidden public raw-row escape, repeated row→ID loop, or bridge regression.
- Each part independently satisfies its exact structured inventory and D-021 defaults; Parts 1, 2, and 3A do not change M04-PR02 status, close #1093, or unblock dependents.
- Performance evidence separates Part 1 representation/lifecycle overhead, Part 2 producer/binding/read overhead, Part 3A bridge-removal/internal-candidate overhead, and final Part 3B downstream overhead.
...
- Any implementation part requires a production path absent from its binding `production_paths` inventory, more than 25 production files, or more than 1,500 net non-generated lines; stop/replan rather than grant an implicit exception or add a fifth or implicit extra implementation part under this record.
- Part 1 requires a row bridge through the stable `selene-db` facade, an advertised compatibility promise, graph-producer migration beyond the named lifecycle minimum, or any bridge beyond the named Part 3A graph-internal bridge; stop/replan rather than broaden Part 1.
...
- Part 3A requires another production path, cannot delete both graph-internal bridge methods without an equivalent bridge/facade crossing, or cannot preserve required checks and semantics; stop/replan rather than broaden Part 3A or advance bridge state.
- Part 3B requires another production path, cannot preserve the internal-only dense projection boundary, cannot delete every remaining repository-public graph raw-row surface/repeated row→ID loop/legacy adapter, or cannot supply final product-shaped/API/lint/full evidence; stop/replan rather than claim completion or close #1093.
