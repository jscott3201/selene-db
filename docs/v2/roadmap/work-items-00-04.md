...
- Existing `VectorCandidateSet` remains an explicitly unbound sorted/deduplicated stable-`NodeId` input helper outside `CandidateSet<Node>`, the graph/generation/layout binding, and the temporary row bridge; upper callers bind its IDs through the pinned graph snapshot before typed graph-owned candidate use, and Part 2 does not delete it.
...
- Part 1: layout-identity regressions cover independent graph runtimes with the same `GraphId` and generation plus compaction/remap without a generation advance; old candidates reject on layout mismatch, while freshly produced candidates yield the same surviving stable IDs.
...
- Part 2: algorithms/GQL/facade/testing candidate, optimizer, projection, query, and index regression suites cover facade `DatabaseId` ingress and binding unbound `VectorCandidateSet` IDs through a pinned snapshot, plus the dependency-direction audit.
...
- Part 2: all downstream algorithms/GQL/facade/testing adapters and private projections migrate with facade `DatabaseId` validation and pinned-snapshot binding for unbound `VectorCandidateSet` IDs, without dependency inversion; no hidden repository-public raw-row escape or bridge remains.
