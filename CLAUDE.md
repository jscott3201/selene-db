...
### D13 — Snapshot section tag width: 8 bytes (provider + sub-tag); section table row 24 bytes (2026-05-07)

The snapshot file's TLV section table now uses an 8-byte composite tag per row: a 4-byte provider tag concatenated with a 4-byte sub-tag. Row width is 24 bytes (8B tag + 8B offset + 8B length). The engine routes each section to its registered `IndexProvider` by `(provider_tag, sub_tag)` without parsing payload, so providers like `selene-vector` can declare independent sections for HNSW topology, centroid index, and raw vector data. Cost: 4 extra bytes per section table row, negligible at hundreds-of-sections scale. Long-term flexibility wins: the format does not need a version bump if a provider adds a new sub-tag. Snapshot layout details live in `_spec/04-persistence-format.md` §4.2.

### D14 — Snapshot serialization: rkyv archive over sorted-vec intermediates (2026-05-07)

imbl 7.0's HAMT and RRB-Tree types are not archivable by rkyv directly. selene-db snapshots imbl-shaped state (`imbl::HashMap<K, V>`, `imbl::Vector<T>`) as `Vec<(K, V)>` and `Vec<T>` sorted by key. rkyv archives the sorted vec natively; the snapshot file mmaps zero-copy. On cold start, the engine bulk-constructs imbl from the archived sorted iterator via `imbl::HashMap::from_iter`/`imbl::Vector::from_iter` — O(n) over already-sorted data, bounded and predictable (millions of nodes typically rebuild in <100 ms). Honest framing: "zero-copy file, fast materialization" rather than "zero-copy entire graph." If a future imbl release ships rkyv archive support natively, adopting it is a non-breaking optimization (snapshot format version stays the same; the rebuild step becomes a no-op). D9's rkyv-for-snapshot decision stays intact; only the prose around "zero-copy" is corrected. Snapshot serialization details live in `_spec/04-persistence-format.md` §4.3.

### D15 — Recovery API: two-step (snapshot apply, then WAL replay); IndexProvider has `read_section`, `write_section`, `on_change`; spec 06 is canonical (2026-05-07)
...
**D15 amendment — 2026-05-07 — IndexProvider receivers flip to `&self`**

The original `&mut self` shape on `read_section` and `on_change` is structurally impossible to honor with the `Vec<Arc<dyn IndexProvider>>` storage from spec 03 §3.1: `Arc<dyn Trait>` cannot deliver `&mut`. BRIEF-08 amends spec 06 §3 to use `&self` everywhere; providers use interior mutability (`parking_lot::Mutex`, `RwLock`, atomics) for any mutable state. The engine guarantees serialized calls per graph (single-writer, single-thread recovery), so a single mutex per provider is sufficient. No safety guarantee is lost; the spec catches up to Rust's storage model.

**D15 follow-up — 2026-05-08 — recovery-side trait split into `RecoveryProvider` (BRIEF-12)**

D15 originally framed recovery as calls into `IndexProvider::read_section` and `IndexProvider::on_change`. BRIEF-12 instead introduced a separate `RecoveryProvider` trait in `selene-persist`, because spec 04 §9 forbids selene-persist from depending on selene-graph (where `IndexProvider`, `ProviderTag`, `SubTag`, and `ProviderError` live). The recovery-side trait uses raw `[u8; 4]` for both tags, returns `RecoveryResult<T> = Result<T, Box<dyn Error + Send + Sync + 'static>>`, and ships only the methods the recovery orchestrator (spec 04 §5) actually invokes: `provider_tag(&self) -> [u8; 4]`, `read_section(sub: [u8; 4], bytes: &[u8])`, and `on_change(&Change)`. No `write_section`, no `declared_sub_tags`. An extension that participates in both live commits and recovery implements both traits — see spec 06 §7.1 for the two recommended patterns (shared inner / coalesced impl). The two-trait split preserves selene-persist's graph-blindness without losing the unified-Change-stream contract. Spec 06 §12 has the full amendment text.
