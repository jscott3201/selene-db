...
Active implementation. Architecture is settled (D1–D21, see Decision log). Workspace contains **all 7 of 7 v1.0 mandatory crates**: `selene-core`, `selene-graph`, `selene-persist`, `selene-gql`, `selene-testing`, `selene-pack`, `selene-algorithms`. **M5d (executor) closed 2026-05-10**; **M5e (procedure-pack registry) closed 2026-05-11** (9/9 briefs, BRIEF-41 through BRIEF-49); **M5f (selene-algorithms) closed 2026-05-11** (7/7 briefs, BRIEF-50 through BRIEF-56). **v1.0 mandatory-crate set is feature-complete.** **M8 (selene-vector opt-in extension) closed 2026-05-12** (8/8 briefs, BRIEF-57 through BRIEF-64). **M9 (selene-vector vector-quality) closed 2026-05-13** (6/6 briefs, BRIEF-65 through BRIEF-70). **M10 (selene-algorithms-pack) closed 2026-05-14** (6/6 briefs, BRIEF-71 + BRIEF-72 + BRIEF-73 + BRIEF-74 + BRIEF-75 + BRIEF-76 merged: scaffold + builder-only external-pack API in selene-pack + graph-scoped `ProjectionCatalog` + `algo.projection_*` family + `algo.pagerank` reference adapter + 7 structural adapters `algo.wcc` / `algo.wcc_count` / `algo.scc` / `algo.scc_count` / `algo.topological_sort` / `algo.articulation_points` / `algo.bridges` + 3 pathfinding adapters `algo.dijkstra` (single-pair, single-row `[cost, path: LIST<NodeRef>, length]`) / `algo.sssp` (single-source) / `algo.apsp` (all-pairs, `max_nodes: INTEGER`) + `algo.betweenness` centrality adapter (nullable `sample_size: INTEGER`; ignores projection edge weights) + 3 community adapters `algo.label_propagation` (nullable `max_iter: INTEGER`; ignores weights) / `algo.louvain` (nullable `max_iter: INTEGER`; uses projection weights; `level: UINT64` reserved for hierarchical mode) / `algo.triangle_count` (no params; ignores weights; DESC-by-count order) + community-ID `Value::NodeRef` projection for direct `MATCH (n) WHERE n = community` round-trip + shared `with_algorithm_projection` helper + `nullable_option_usize` helper preserving `NULL` vs `Some(0)` + `NodeRef`-input analyzer rejection + corpus-harness closeout (D21 snapshot battery + criterion bench seed)). **M11 (selene-vector-pack) in progress 2026-05-14** (1/~5 briefs, BRIEF-77 merged: `selene-vector-pack` scaffold + `vector.search` HNSW read-tier adapter + `vector_pack_corpus` mirror + `IndexProvider::as_any` upcast surface + `GraphContext` provider-slice plumbing). v1.x backlog: weighted-betweenness adapter, hierarchical Louvain, `algo.louvain_modularity`, parallel execution (rayon), streaming/incremental algorithms, out-of-core for graphs exceeding in-memory CSR bounds; opt-in extension crates (selene-timeseries, selene-rdf, selene-graphrag, selene-fulltext). See `_design/milestone-log.md` for detailed entries (canonical state in MCP project 364).
...
| Crate | Depends on | Owns |
|---|---|---|
| `selene-vector` | core, graph | HNSW vector index provider (`VECT` with sub-tags `GRPH` / `VECS` / `QUNT`), vector procedure-pack manifest stub (`pack_name="vector"`), ArcSwap-published `HnswGraph` snapshot, scalar distance kernels (cosine / L2 / dot), HNSW insert + search + RoaringBitmap pre-filter, GRPH+VECS+QUNT rkyv snapshot codecs (with legacy decoders for backward compat), VECB bulk-insert wire format, SQ8 + PQ quantization (asymmetric ADC search), OPQ rotation (Procrustes-trained, dim ≤ `OPQ_MAX_DIM`), HNSW diversity-aware neighbor selection, IVF-PQ provider (`IVFP` with sub-tags `CQNT` / `IPQB` / `POST`) with residual-PQ posting-list search, and D21 snapshot harness. Procedure registration moves to future `selene-vector-pack` work; polysemous codes and HNSW+PQ composition remain in M9 backlog. |
...
Detailed milestone-progress entries (BRIEF-by-BRIEF closure notes, file paths, P-level review findings, fix commits, spec amendments, test counts) live in **`_design/milestone-log.md`** (local-only working document; reproducible from MCP project 364).

The MCP graph is the canonical source of truth:

- `selene_db` project node 364 → contains `milestone` nodes (one per M5* phase) → each `contains_brief` edges to `brief` nodes (BRIEF-41+).
- Brief nodes carry status (`drafted` / `brief_back_resolved` / `merged`), PR number, base commit, merge commit, fix commit, scope summary, key design decisions, Codex review findings, and spec amendment range.

When the in-tree `_design/milestone-log.md` view drifts from the graph, regenerate it from the graph (the graph wins).
